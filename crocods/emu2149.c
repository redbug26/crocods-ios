...
signed short calc (PSG * psg);

void internal_refresh (PSG * psg) {
  if (psg->quality) {
    psg->base_incr = 1 << GETA_BITS;
    psg->realstep = (unsigned int) ((1 << 31) / psg->rate);
    psg->psgstep = (unsigned int) ((1 << 31) / (psg->clk / 16));
    psg->psgtime = 0;
  }
  else {
    psg->base_incr = (unsigned int) ((double) psg->clk * (1 << GETA_BITS) / (16 * psg->rate));
  }
}
...
/*
EMU2149_API PSG *PSG_new (unsigned int c, unsigned int r) {
  PSG *psg;
...
EMU2149_API unsigned int PSG_setMask (PSG *psg, unsigned int mask) {
  unsigned int ret = 0;
  if (psg) {
    ret = psg->mask;
    psg->mask = mask;
...
EMU2149_API unsigned int
PSG_toggleMask (PSG *psg, unsigned int mask)
{
  unsigned int ret = 0;
  if(psg)
  {
...
EMU2149_API unsigned char
PSG_readIO (PSG * psg)
{
  return (unsigned char) (psg->reg[psg->adr]);
}

EMU2149_API unsigned char
PSG_readReg (PSG * psg, unsigned int reg)
{
  return (unsigned char) (psg->reg[reg & 0x1f]);

}

/*
EMU2149_API void
PSG_writeIO (PSG * psg, unsigned int adr, unsigned int val)
{
  if (adr & 1)
...
signed short calc (PSG * psg) {
  ...
  return (signed short) mix;
}

EMU2149_API signed short PSG_calc (PSG * psg) {
  if (!psg->quality)
    return (signed short) (calc (psg) << 4);
  ...
  return (signed short) (psg->out << 4);
}

/*
EMU2149_API void PSG_writeReg (PSG * psg, unsigned int reg, unsigned int val) {
  int c;

  if (reg > 15) return;

  psg->reg[reg] = (unsigned char) (val & 0xff);
  switch (reg)
  {
  case 0:
  case 2:
